...
* A Pass Through Proxy and the `get` trap
  ```js
  ...
  agent2.status; // logs out the richard object (not the agent object!) and the name of the property being accessed (`status`)
  ```
